...
func PrivateHandler(w http.ResponseWriter, r *http.Request) {
	...
	if isEnrolled != 0 {
		if isEnrolled == evnt {
			http.Error(w, "Player Registered Successfully!", http.StatusAlreadyReported)
		} else {
	...
	if prevEventId != 0 {
		http.Error(w, "Player Registered this event before!", http.StatusOK)
		return
	}
	...
	groupId, categoryId, err = GetCategoryForLevel(tx, ctx, lvl, evnt)
	...
	if groupId == 0 {
		...
		IncreaseGroupProcess(eventNo + "|" + strconv.Itoa(groupId))
	...
	key := eventNo + "|" + strconv.Itoa(groupId)
